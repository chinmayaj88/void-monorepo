...
export interface IFileRepository {
    ...
    // Access tracking
    updateLastAccessedAt(fileId: FileId): Promise<void>;
    incrementAccessCount(fileId: FileId): Promise<void>;
    // Archive operations
    updateStorageTier(fileId: FileId, storageTier: 'standard' | 'archive'): Promise<void>;
    findFilesToArchive(daysThreshold: number): Promise<File[]>;
    // Storage stats
    getStorageStats(ownerId: UserId): Promise<{
        totalFiles: number;
        totalSize: number;
        totalStandardSize: number;
