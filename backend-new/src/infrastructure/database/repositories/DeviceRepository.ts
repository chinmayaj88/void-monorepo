...
export class DeviceRepository implements IDeviceRepository {
    ...
    async findByVerificationToken(token: string): Promise<Device | null> {
        const result = await this.pool.query<DeviceRow>(
            'SELECT * FROM devices WHERE verification_token = $1 AND revoked_at IS NULL',
            [token]
        );
        return this.mapFirstRow(result);
    }
    ...
    async countDevices(userId: UserId): Promise<number> {
